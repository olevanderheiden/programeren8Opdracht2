...
const predctionText = document.getElementById('prediction')
...
function clickHandler(e)
{
    ...
    {
        machine.learn(currentHandPositionNeuteral, e.target.innerHTML)
    }
}

function neutralisatorEX(rawData)
{
    let outputs = [];

    for (const rawPoint of rawData[0].landmarks) {
        let x = rawPoint[0]
        let y = rawPoint[1]
        outputs.push([x, y])
    }
    for (let index = 0; index < outputs.length; index++) {
        outputs[index] = [(outputs[index][0] - rawData[0].boundingBox.topLeft[0]), (outputs[index][1] - rawData[0].boundingBox.topLeft[1])]
    }
    currentHandPositionNeuteral = outputs.flat();
    let prediction = machine.classify(currentHandPositionNeuteral)
    if (prediction !== undefined)
    {
        console.log(`Ik denk dat het een ${prediction} is!`)
        predctionText.innerHTML = `Ik denk dat het een ${prediction} is!`
    }
}
